...
const MobileTabs = () => {
    ...
    return (
        <div className='lg:hidden bg-white shadow-lg p-3 fixed bottom-0 z-10 w-full flex items-center justify-between md:justify-evenly text-gray-500 border'>
            {allTypes.map((item) => (
                <Link to={`/${item.id}`} className='w-1/4'>
                    <div className={
                        type === item.id ? "flex flex-col items-center text-xl text-red-400 text-center" : "flex flex-col items-center text-xl"
                    }>
                        <div className={
                            type == item.id ? "h-full w-full border-t-2 p-3 border-red-500 flex flex-col items-center" : "flex flex-col items-center "
                        }>
                            {item.icon}
                    ...
                    <Link to={`/${items.id}`}>
                        <div
                            className={classnames(
                                ...
                                {
                                    "border-b-2 border-red-400": type === items.id,
                                }
                            )}
                        >
                            <div
